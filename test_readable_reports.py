...
def test_readable_reports():
    base_url = "http://localhost:8001"
    
    print("Testing Readable Reports Backend")
    print("=" * 50)
...
2024-01-03,Chicken 1kg,Meat,1200,80,96000"""
    ...
    try:
        ...
        # Summary
        print("\n" + "=" * 50)
        print("READABLE REPORTS SUMMARY")
        print("=" * 50)
        ...
        if passed == total:
            print("SUCCESS: ALL READABLE REPORTS ARE WORKING!")
        else:
        ...
        # Show some HTML files if they exist
        html_files = [f for f in os.listdir('exports') if f.endswith('.html')]
        if html_files:
            print(f"\nHTML files generated: {len(html_files)}")
            for html_file in html_files[-3:]:  # Show last 3 HTML files
                print(f"  - {html_file}")
